...
- **AWS Route 53 Resolver Profiles** (GA 2023-11): bundle resolver rules and DNS Firewall configs and share to many VPCs via RAM, removing per-VPC association toil ([Route 53 Resolver Profiles](https://aws.amazon.com/about-aws/whats-new/2023/11/amazon-route-53-resolver-profiles/)).
...
**Kubernetes north-south:** for new platform work use **Gateway API**, the upstream-blessed successor to Ingress. Ingress is feature-frozen ([Gateway API](https://gateway-api.sigs.k8s.io/)). Implementation choice (Istio gateway, Cilium, Envoy Gateway, cloud-managed) and the L7 policy on top live in **ch04 §9**; this chapter only notes that the L4/L7 transport surface now lands on `Gateway`/`HTTPRoute` resources, not `Ingress`.
...
- **Default CNI for greenfield: Cilium.** CNCF graduated 2023-10-11 (the first graduated cloud-native networking project), eBPF dataplane, replaces kube-proxy, native NetworkPolicy + CiliumNetworkPolicy with identity-aware policy, transparent WireGuard or IPsec encryption, Hubble for flow observability ([CNCF — Cilium graduation](https://www.cncf.io/announcements/2023/10/11/cloud-native-computing-foundation-announces-cilium-graduation/); [Cilium](https://docs.cilium.io/)). Calico is a fine alternative when eBPF is undesired or kernel constraints apply.
